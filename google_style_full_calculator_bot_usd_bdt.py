...
async def crypto_price(text: str):
    ...
    # Fetch USD + BDT together so the result always shows both.
    cache_key = (coin_id, "usd_bdt")
    ...
    bdt_price = info.get("bdt")
    if usd_price is None or bdt_price is None:
        return None

    usd_total = quantity * float(usd_price)
    bdt_total = quantity * float(bdt_price)
    change = info.get("usd_24h_change")

    change_text = ""
    if change is not None:
        sign = "+" if float(change) >= 0 else ""
        change_text = f"\n24h: {sign}{float(change):.2f}%"

    # Google-style compact result with both USD and BDT.
    return (
        f"{_crypto_amount(quantity)} {coin_name} ({symbol.upper()}):\n"
        f"${_money(usd_total)} USD\n"
        f"৳{_money(bdt_total)} BDT"
        f"{change_text}"
    )
