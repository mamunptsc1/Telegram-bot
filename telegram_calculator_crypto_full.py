...
# CoinGecko public API (no API key required for this public endpoint)
# ============================================================
...
async def get_coin_list():
    ...
    if COIN_CACHE and (time.time() - COIN_CACHE_TIME) < COIN_CACHE_TTL:
        return COIN_CACHE

    async with httpx.AsyncClient(timeout=15) as client:
        r = await client.get(f"{COINGECKO_BASE}/coins/list")
        r.raise_for_status()
        data = r.json()
    ...
    for coin in data:
        ...
        if not cid:
            continue
        # Prefer the first symbol match; exact names are also supported.
...
def crypto_query_parts(text: str):
    ...
    # Examples: btc, 1 btc, 5*0.7 eth, 2.5 bitcoin
    m = re.fullmatch(r"(.+?)\s+([a-zA-Z0-9][a-zA-Z0-9._-]*)", t)
    if not m:
...
async def crypto_price(text: str):
    ...
    coin_id = coins.get(coin_query)

    if not coin_id:
        return None

    # Evaluate quantity, e.g. 5*0.7 eth -> 3.5 ETH.
    try:
    ...
    info = data[coin_id]
    unit_price = float(info[fiat])
    total = quantity * unit_price
    change = info.get(f"{fiat}_24h_change")
    market_cap = info.get(f"{fiat}_market_cap")
    volume = info.get(f"{fiat}_24h_vol")

    symbol = coin_query.upper()
    lines = [
        f"🪙 {coin_id.replace('-', ' ').title()}",
        f"💰 1 {symbol} = {unit_price:,.12g} {fiat.upper()}",
        f"🧮 {quantity:,.12g} {symbol} = {total:,.12g} {fiat.upper()}",
    ]

    if change is not None:
        sign = "+" if float(change) >= 0 else ""
        lines.append(f"📈 24h: {sign}{float(change):.2f}%")
    if market_cap is not None:
        lines.append(f"🏦 Market Cap: {float(market_cap):,.12g} {fiat.upper()}")
    if volume is not None:
        lines.append(f"📊 24h Volume: {float(volume):,.12g} {fiat.upper()}")
...
async def top_crypto(limit=10):
    ...
    for i, coin in enumerate(data, 1):
        ...
        lines.append(f"{i}. {coin.get('name')} ({symbol}) — ${price:,.12g} | 24h {change_text}")
